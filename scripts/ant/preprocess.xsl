...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="//tei:respStmt//tei:persName[@xml:id='ampf']">
        <xsl:copy>
            <xsl:attribute name="ref">
                <xsl:text>https://orcid.org/0000-0002-7722-4091</xsl:text>
            </xsl:attribute>
            <xsl:apply-templates select="node()|@*"/>
        </xsl:copy>
    </xsl:template>
     <xsl:template match="//tei:respStmt//tei:persName[@xml:id='fs']">
        <xsl:copy>
            <xsl:attribute name="ref">
                <xsl:text>https://orcid.org/0000-0002-8815-6741</xsl:text>
            </xsl:attribute>
            <xsl:apply-templates select="node()|@*"/>
        </xsl:copy>
    </xsl:template>
