...
if __name__ == "__main__":
    ...
    for file in fileDirList:
        ...
        if len(origFileExtension) == 0:
            ...
            if (
                origFileName.decode("utf-8").rsplit(".", 1)[1].lower()
                in ignoreFileExtensions
            ):
                print(
                    "ignoring: "
                    + os.path.join(
        ...
        newFilePath = os.path.join(oldPath.decode("utf-8"), f"v__{newFile}")
        ...
        if rename == "true":
            try:
                ...
                print(
                    "The file could not be renamed, because a file with the same name already exists!"
                )
