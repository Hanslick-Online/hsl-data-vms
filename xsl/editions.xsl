...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <xsl:variable name="file" select="replace(replace(replace(replace(translate(replace(//tei:titleStmt/tei:title[1]/text(), '[\s,]', '_'), '.', ''), '[()]', ''), 'ö', 'oe'), 'ü', 'ue'), 'ä', 'ae')"/>
    ...
    <xsl:template match="tei:titleStmt">
        ...
        <editionStmt xmlns="http://www.tei-c.org/ns/1.0">
            <edition>Der Kritiker in der Kritik: Die Rezensionen zu Eduard Hanslicks Traktat „Vom Musikalisch-Schönen“ (1854–1857)</edition>
            <respStmt>
                <resp>Herausgegeben von</resp>
                <name ref="https://orcid.org/0000-0002-0117-3574">Wilfing, Alexander</name>
            </respStmt>
